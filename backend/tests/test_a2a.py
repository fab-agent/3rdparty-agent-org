...
def a2a_setup(auth_client, db_session):
    ...
    responsible = make_personnel(db_session, co.id, name="Responsible Human",
                                  slug="responsible-human", type="human", title="Lead")
    responsible.user_id = human.id

    from_agent = make_personnel(db_session, co.id, name="FromBot",
                                 slug="from-bot", type="agent", title="Sender Agent")
    to_agent = make_personnel(db_session, co.id, name="ToBot",
                               slug="to-bot", type="agent", title="Receiver Agent")

    from_cfg = make_agent_config(db_session, from_agent.id, model="gpt-4o-mini",
                                  responsible_id=responsible.id)
    to_cfg = make_agent_config(db_session, to_agent.id, model="gpt-4o-mini",
                                responsible_id=responsible.id)
...
def _post_request(client, setup, task="Test görevi", **extra):
    ...
    return client.post("/a2a/requests", json={
...
def test_create_a2a_request(auth_client, a2a_setup):
    r = _post_request(auth_client, a2a_setup,
...
def test_create_request_invalid_agent(auth_client):
    r = auth_client.post("/a2a/requests", json={
...
def test_reject_request(auth_client, a2a_setup):
    ...
    r = auth_client.post(f"/a2a/requests/{req['id']}/reject", json={
...
def test_cannot_reject_already_rejected(auth_client, a2a_setup):
    ...
    auth_client.post(f"/a2a/requests/{req['id']}/reject", json={
        "approver_id": a2a_setup["responsible_id"], "reason": "Reddedildi",
    })
    r = auth_client.post(f"/a2a/requests/{req['id']}/reject", json={
        "approver_id": a2a_setup["responsible_id"], "reason": "Tekrar red",
    })
...
def test_approve_request_transitions_to_running(auth_client, a2a_setup):
    ...
    with patch("api.a2a._execute_a2a_task"):
        r = auth_client.post(f"/a2a/requests/{req['id']}/approve", json={
            "approver_id": a2a_setup["responsible_id"],
        })
    assert r.status_code == 200
...
def test_cannot_approve_non_pending(auth_client, a2a_setup):
    req = _post_request(auth_client, a2a_setup, task="Görev").json()
    with patch("api.a2a._execute_a2a_task"):
        auth_client.post(f"/a2a/requests/{req['id']}/approve", json={
            "approver_id": a2a_setup["responsible_id"],
        })
        r = auth_client.post(f"/a2a/requests/{req['id']}/approve", json={
            "approver_id": a2a_setup["responsible_id"],
...
def test_full_a2a_flow(auth_client, a2a_setup):
    ...
    # 1. Create
    req = _post_request(auth_client, a2a_setup,
                        task="Deploy raporu hazırla",
                        context="Son sprint sonuçları").json()
    ...
    # 5. Approve result
    r = auth_client.post(f"/a2a/requests/{req['id']}/approve-result", json={
        "approver_id": a2a_setup["responsible_id"],
    })
    assert r.status_code == 200
