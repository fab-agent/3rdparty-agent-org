...
def create_git_config(body: GitConfigCreate, company_id: str | None = None):
    with get_session() as session:
        ...
        if existing:
            raise HTTPException(
                status_code=409,
                detail="Bu şirket için zaten bir git bağlantısı mevcut. Önce silin.",
            )
...
def update_git_config(body: GitConfigUpdate, company_id: str | None = None):
    with get_session() as session:
        ...
        if body.token is not None:
            cfg.encrypted_token = encrypt(body.token)
        if body.sync_interval is not None:
            cfg.sync_interval = body.sync_interval
        if body.auto_pr is not None:
            cfg.auto_pr = body.auto_pr
        session.add(cfg)
        log_action(
            session,
            "update",
...
def git_pull(company_id: str | None = None):
    with get_session() as session:
        ...
        log_action(
            ...
            entity_name=cfg.repo_url,
            company_id=cfg.company_id,
            details={"direction": "pull", "status": log.status},
        )
        session.commit()
...
def git_push(body: PushRequest, company_id: str | None = None):
    with get_session() as session:
        ...
        log_action(
            ...
            entity_name=cfg.repo_url,
            company_id=cfg.company_id,
            details={"direction": "push", "status": log.status},
        )
        session.commit()
        return _log_to_dict(log)
